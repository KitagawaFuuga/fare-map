...
`app/page.tsx`（仮実装。Task 13 で置換する）:
...
// 10km ごとに 100 円/初乗り 100 円の単純ルールでテスト
...
describe("findReachable", () => {
  it("同一事業者は距離合算で運賃を出す（A→C は 200 円であり 100+100 ではない…この例では同額だが表で検証）", () => {
    const result = findReachable(graph, calc, "A", 10000);
    const c = result.find((r) => r.id === "C");
    expect(c?.fare).toBe(200); // 20km を 1 区間として表引き
  });

  it("事業者が変わると区間を分けて加算する", () => {
    const result = findReachable(graph, calc, "A", 10000);
    const d = result.find((r) => r.id === "D");
    expect(d?.fare).toBe(300); // OpA 20km = 200 + OpB 10km = 100
  });

  it("予算内の駅だけ返す", () => {
    const result = findReachable(graph, calc, "A", 200);
