...
**Files:**
- Create: `scripts/pipeline/fetch-data.ts`, `scripts/pipeline/build-graph.ts`, `scripts/pipeline/validate-graph.ts`, `scripts/pipeline/README.md`, `data/graph.json`（生成物）
- Modify: `package.json`（`pipeline:fetch` スクリプトを追加）

**Interfaces:**
- Consumes: `parseCsv` / `buildGraph` / `validateGraph`
- Produces: `data/graph.json`（`RailGraph` の JSON。以降のタスクはこのファイルの存在に依存）

- [ ] **Step 1: 取得スクリプトと README を書く**

データは ny-a/ekidata（駅データ.jp が利用規約で認めた再配布ミラー）から取得する。会員登録不要で `curl` / `fetch` で直接落とせる。
...
`package.json` の scripts に追加: `"pipeline:fetch": "tsx scripts/pipeline/fetch-data.ts"`
...
Run: `npm run pipeline:fetch && npm run pipeline:build && npm run pipeline:validate`
Expected: fetch は 4 ファイルの行数を出力（station.csv が約 10,865 行）。build は nodes 約 10,478、edges 数万のオーダー。validate の警告は内容を確認し、レポートに件数と代表例を記載する（貨物駅・支線末端などの孤立ノードは許容。NaN 距離エッジが 1 件でも出たら異常なので報告すること）
