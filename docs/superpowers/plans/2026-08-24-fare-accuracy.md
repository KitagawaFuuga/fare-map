...
## 第2次改善（調査 `research-transfer.md` を受けて）

実測 12 区間の結果、誤差の傾向は次のとおり:
- JR 会社跨ぎ: 安く出る（-15.5%〜+1.2%）
- 私鉄の相互直通: 高く出る（+13.9%〜+86.5%）
- 単一事業者・短距離: ほぼ正確

### Task 7: 主要私鉄の運賃表を追加

**背景:** 探索は「最安ルート」を選ぶため、運賃表の無い事業者が競合区間で選ばれると誤差がそのまま表示される。新宿→高尾が 590 円と出るのは京王電鉄（運賃表なし＝汎用表）経由が選ばれるためで、実運賃は 410 円。

**対象（駅数順・運賃表が無いもの）:** 富山地方鉄道(121) / 東急電鉄(114) / 伊予鉄道(110) / 京阪電鉄(96) / 京成電鉄(80) / 京急電鉄(77) / 京王電鉄(76) / 西日本鉄道(74) / 札幌市交通局(73)

**優先度:** 首都圏・関西の競合区間に関わる **京王・京急・京成・京阪・東急** を最優先。地方の事業者は後回しでよい。

手順は既存の `data/fare-rules/*.json` と同じ（公式出典 → JSON → `source` に出典記録 → **実運賃との完全一致テスト**を各社 3 件以上）。

**注意:**
- 東急電鉄は世田谷線（均一運賃）とこどもの国線が本線（対キロ制）と同一事業者名で混在するため、以前見送った。**本線用の距離表を入れたうえで、均一運賃区間は `fare-overrides` の駅ペアで個別に上書きする**ハイブリッド方式を検討すること。それが困難なら引き続き見送り、理由を記載する
- 京急・京成は空港線に加算運賃がある。テスト区間は加算運賃の対象外を選ぶこと
- 許容誤差は使わない。完全一致にできない区間はテストに使わない
- 出典が確認できない事業者は入れず、レポートに記載する

### Task 8: 100km 超の運賃を公式の基準額表に置き換える

**背景:** 公式の基準額表は 20km 刻みの階段テーブルだが、現行の `beyond` は `baseFare + (km - fromKm) * ratePerKm` の線形式。250.8km で公式 4,510 円に対し 4,140 円（-8%）。**会社跨ぎ以前に、単一事業者の長距離運賃がずれている。**

**データ源（調査で取得済み）:**
`http://web.archive.org/web/20260306174402/https://www.jreast.co.jp/2026unchin-kaitei/assets/pdf/kijun_kasan_futsuu.pdf`
（元 URL は bot 保護で 403。Wayback 経由で全文取得可能）
判明済みの値: 91〜100km=1,690 / 101〜120km=1,980 / 121〜140km=2,310 / … / 241〜260km=4,510 / 261〜280km=4,840 / 281〜300km=5,170 / 301〜320km=5,500

**方針:** `table` を 100km 超まで延長して階段表をそのまま持たせる。`beyond` は表の上限を超えた場合のみ使う。JR 各社ぶんを更新すること（どの社が基準額表を使うかは出典で確認）。

検証: 250.8km・293.6km・303.9km などで公式値と**完全一致**すること。既存の 100km 未満のテストが壊れないこと。

### Task 9: JR 会社跨ぎを「通算加算方式」に対応する

**背景:** 2026年3月14日改定で新設。`運賃 = 基準額（総営業キロで表を1回引く）+ 加算額（JR東日本区間の営業キロぶんの上乗せ）`。現行実装は会社境界で初乗りをやり直すため方向性が逆で、新宿→豊橋は 4,370 円（実運賃 5,170 円、-15.5%）。

**設計上の要点:**
- `SearchState` に「JR 本州3社を通算中の距離」と「うち JR東日本区間の距離」を保持する必要がある
- 事業者切り替えの扱いを **「JR 本州3社どうし」「JR ⇔ 私鉄」「私鉄どうし」** で区別する。前者は区間を確定せず通算を継続する
- 加算額表も `data/fare-rules/` に持たせる
- **既存の単一事業者の運賃計算を壊さないこと**（テスト 110 件が通り続けること）

検証: 新宿→豊橋・東京→名古屋・東京→大阪が実運賃と一致すること。単一事業者の既存テストが壊れないこと。

**未解明の点（調査レポート 5 節）:** 新宿→豊橋で加算額が上乗せされない理由が不明（特定都区市内ルールとの組み合わせと推定）。実装前に追加調査するか、実測に合う形を採用して根拠をレポートに明記すること。

**三島会社（北海道・四国・九州）は対象外**とする。加算額表の数値が未確認であり、影響駅数も少ないため。

### Task 10: 私鉄相互直通の追加調査（調査のみ・コード変更なし）

**背景:** 横浜→和光市 +60.7%、羽田空港→押上 +86.5% と誤差が最大だが、**根本原因が未特定**。乗継割引（メトロ⇔都営 70円 / 東急⇔メトロ 10〜20円）では説明できない規模。

**調べること:** 各社（東急・東京メトロ・東武・京急・京成・都営交通局）の運輸連絡協定・連絡運賃のページを個別にあたり、直通運転区間に**通し運賃表が別建てで存在するか**を確認する。存在するならその形式（距離制か駅ペアか）と、データ構造の提案まで。

原因が特定できるまで実装しないこと。誤った仕組みを作ると既存の精度を壊す。
